...
```{r}
max(abs(lm(los ~ gender + age + smoker + exercise, dat=x)$coefficients[-1]))
# Gender male has the highest effect on los.
```
...
```{r}
coef(summary(mod))
# The estimate for the intercept is 3.5, and the estimate for gender male is 4.3.
```
...
```{r}
2*pt((mod.c[,1]/mod.c[,2])[2],7,lower.tail = F)
```
...
```{r}
sqrt(deviance(mod)/7)==summary(mod)$sigma
```
...
# The second line of code yields the same p value as in the model summary.
```
